/**
 * Template string `${}`
 * innerHTML ou template
 * += operator
 */

// 1. Je recupere le conteneur de publications
const postsContainer = document.querySelector(".posts");

// 2. Voici ma base de données factise 
const posts =
    [
        {
            titre: "SEO, les bonnes pratiques",
            hashtag: "#SEO",
            link: "#",
            extrait: "Mollit ut mollit esse exercitation nisi ut labore velit anim pariatur sit deserunt anim. Dolore consequat aliquip esse elit culpa aliqua. Consectetur mollit irure minim incididunt nulla non. Ad sunt mollit aliqua minim fugiat et minim commodo. Anim proident incididunt veniam duis cupidatat irure eu. Elit nulla nisi ea laborum mollit excepteur enim ut Lorem. Cupidatat minim consectetur mollit in ut consectetur est duis do sint cillum nisi."
        },
        {
            titre: "Bien, les bonnes pratiques",
            hashtag: "#JS",
            link: "#",
            extrait: "Mollit ut mollit esse exercitation nisi ut labore velit anim pariatur sit deserunt anim. Dolore consequat aliquip esse elit culpa aliqua. Consectetur mollit irure minim incididunt nulla non. Ad sunt mollit aliqua minim fugiat et minim commodo. Anim proident incididunt veniam duis cupidatat irure eu. Elit nulla nisi ea laborum mollit excepteur enim ut Lorem. Cupidatat minim consectetur mollit in ut consectetur est duis do sint cillum nisi."
        },
        {
            titre: "Content, les bonnes pratiques",
            hashtag: "#PHP",
            link: "#",
            extrait: "Mollit ut mollit esse exercitation nisi ut labore velit anim pariatur sit deserunt anim. Dolore consequat aliquip esse elit culpa aliqua. Consectetur mollit irure minim incididunt nulla non. Ad sunt mollit aliqua minim fugiat et minim commodo. Anim proident incididunt veniam duis cupidatat irure eu. Elit nulla nisi ea laborum mollit excepteur enim ut Lorem. Cupidatat minim consectetur mollit in ut consectetur est duis do sint cillum nisi."
        }
    ];

// 3. Je parcours la bdd
posts.forEach(function (post) {
    const divPost = document.createElement("div");
    divPost.classList.add("post");

    const postTitreDiv = document.createElement("div");
    postTitreDiv.classList.add("post-titre");

    const lienArticle = document.createElement("a");
    lienArticle.setAttribute("a",post.link);
    lienArticle.textContent = post.titre;

    const postExtraitDiv = document.createElement("div");
    postExtraitDiv.classList.add("post-extrait");
    postExtraitDiv.textContent = post.extrait;

    const postHashtagDiv = document.createElement("div");
    postHashtagDiv.classList.add("post-hashtag")
    postHashtagDiv.textContent = post.hashtag;

    postsContainer.appendChild(divPost);
    divPost.appendChild(postTitreDiv);
    postTitreDiv.appendChild(lienArticle);
    divPost.appendChild(postExtraitDiv);
    divPost.appendChild(postHashtagDiv);


});